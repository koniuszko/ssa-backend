...
router.get("/", (req, res) => {
    ...
    Streamer.find().then((streamers) => res.json(streamers)).catch((err) => res.status(400).json(`Failed to get streamers : ${err}`));
});


router.get("/:id", (req, res) => {
    Streamer.findById(req.params.id).then((streamer) => res.json(streamer)).catch((err) => res.status(400).json(`Failed to get streamer : ${err}`));
});
...
router.post("/", async (req, res) => {
    ...
    streamer.save()
        ...
        .catch(err => res.status(400).json({error: `Failed to create streamer : ${err}`}));
});
...
router.put("/:id/vote", (req, res) => {
    ...
    Streamer.findById(req.params.id)
        .then((streamer) => {
            streamer.votes = voteType === "upvote" ? streamer.votes + 1 : streamer.votes - 1;
            streamer.save()
